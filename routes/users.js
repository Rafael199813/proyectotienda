...
async function ExistUsuario(account) {
    ...
    try {
        ...
        if (response.data.id > 0) {
            url = response.data.rol == besodeangel ?
            ...
            user = {
                id: user.data.id,
                panel: response.data.rol == besodeangel ? "/links/PanelAcademia" : "/links/PanelEstudiante",
...
router.post("/signin", async(req, res) => {
    let redirect;
    const user = await ExistUsuario(req.body);
